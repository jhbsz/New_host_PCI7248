...
BOOL CMy9525COMAPDlg::OnInitDialog()
{
	...
	{
		m_Comm.SetCommPort((short)2);	//Set COM2 communication	
...
//	SetTimer(20,1000,FALSE);
	SetTimer(WM_READY_TIMER, 100, NULL);
	UpdateData(FALSE);
...
// 	}
	...
	if (nIDEvent == WM_READY_TIMER)
	{
		if(!bWorking)
		{
			...
			if(winHnd != NULL)
				::PostMessage(winHnd, WM_FT_READY, 0, 0);
		}
	}
...
LRESULT CMy9525COMAPDlg::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	{
// 		if (wParam == WM_NO_ECHO_WPAR_FLAG)
// 			bIsEcho = FALSE;
// 		else
// 			bIsEcho = TRUE;
